...
function registerSW(){
  ...
  navigator.serviceWorker.register('/DesafioHV6/service-worker.js').then(reg=>{
...
window.switchTab=function(tab){
  ['main','hist-view','profile-view','store-view','missions-view','config-view'].forEach(id=>document.getElementById(id).style.display='none');
  const map={hoy:'main',hist:'hist-view',profile:'profile-view',store:'store-view',missions:'missions-view',config:'config-view'};
  document.getElementById(map[tab]||'main').style.display='block';
  const titles={hoy:'Hoy',hist:'Historial',profile:'Perfil & Logros',store:'Tienda',missions:'Misiones',config:'⚙️ Configuración'};
  document.getElementById('top-bar-title').textContent=titles[tab]||'Hoy';
  ['hoy','hist','profile','store','missions','config'].forEach(t=>document.getElementById('dnav-'+t)?.classList.toggle('active',t===tab));
  if(tab==='hist')renderHistory();if(tab==='profile')renderProfile();if(tab==='store')renderStore();if(tab==='missions')renderMissionsPage();if(tab==='config')renderConfigView();
};
